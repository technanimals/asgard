...
import { TyrEncryption, TyrEncryptionNode } from "../index.ts";

Deno.test({
  name: "Should extend TyrEncryption class",
  fn: () => {
    const tyrEncryptionNode = new TyrEncryptionNode();
...
Deno.test({
  ...
  fn: () => {
    const tyrEncryptionNodeA = new TyrEncryptionNode();
    const tyrEncryptionNodeB = new TyrEncryptionNode();
...
Deno.test({
  ...
  fn: async () => {
    ...
    const keyPairA = tyrEncryptionNodeA.generateKeyPair();
    const keyPairB = tyrEncryptionNodeB.generateKeyPair();
    const symmetricKeyA = await tyrEncryptionNodeA.deriveSymmetricKey(
      keyPairA.private,
      keyPairB.public,
    );
    const symmetricKeyB = await tyrEncryptionNodeB.deriveSymmetricKey(
      keyPairB.private,
      keyPairA.public,
    );
...
Deno.test({
  ...
  fn: async () => {
    ...
    const { nonce, ciphertext } = await tyrEncryptionNodeA.encryptMessageFor(
      keyPairA.private,
      keyPairB.public,
      message,
    );
...
Deno.test({
  ...
  fn: async () => {
    ...
    const message = "Hello, world!";
    const { nonce, ciphertext } = await tyrEncryptionNode.encryptMessageFor(
      keyPair1.private,
      keyPair1.public,
      message,
    );
...
Deno.test({
  ...
  fn: async () => {
    ...
    const keyPair = tyrEncryptionNode.generateKeyPair();
    const message = "Hello, world!";
    const { ciphertext } = await tyrEncryptionNode.encryptMessageFor(
      keyPair.private,
      keyPair.public,
      message,
    );
...
Deno.test({
  ...
  fn: async () => {
    ...
    const keyPair = tyrEncryptionNode.generateKeyPair();
    const message = "Hello, world!";
    const { nonce: nonce1, ciphertext: ciphertext1 } = await tyrEncryptionNode
      .encryptMessageFor(
        keyPair.private,
        keyPair.public,
        message,
      );
    const { nonce: nonce2, ciphertext: ciphertext2 } = await tyrEncryptionNode
      .encryptMessageFor(
        keyPair.private,
        keyPair.public,
        message,
